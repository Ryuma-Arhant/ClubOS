...
export default function CreateEvent() {
  ...
  const [form, setForm] = useState({
    name: '', club: user.role !== 'Super Admin' ? 'CS Club' : '',
    date: '', time: '', location: '', category: 'Workshop',
    capacity: '', description: '', status: 'Draft',
  });
  ...
  const inp = err => ({
    ...
    border:`1px solid ${err?'rgba(239,68,68,0.5)':'rgba(245,197,24,0.15)'}`,
    borderRadius:10, padding:'11px 14px', color:'#fff', fontSize:13, outline:'none', transition:'border 0.2s',
  });
  const err = { fontSize:11, color:'#f87171', marginTop:5 };
  ...
  if (saved) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0a0a0a', fontFamily:"'Plus Jakarta Sans',sans-serif", flexDirection:'column', gap:16 }}>
      ...
      <div style={{ fontSize:18, fontWeight:700, color:'#fff' }}>Event {form.status === 'Published' ? 'Published' : 'Saved as Draft'}!</div>
      <div style={{ fontSize:13, color:'#A0A0A0' }}>Redirecting to Events…</div>
    </div>
  );

  return (
    <AppShell navItems={navItems} active="Events" onNav={handleNav} user={user} onLogout={logout} title="Create New Event">
      <div style={{ maxWidth:780, margin:'0 auto' }}>
        ...
        <GlassCard style={{ padding:'32px 36px' }}>
          <h2 style={{ fontSize:18, fontWeight:800, color:'#fff', marginBottom:6 }}>New Event</h2>
          <p style={{ fontSize:13, color:'#A0A0A0', marginBottom:28 }}>Fill in the details below. You can save as draft or publish immediately.</p>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            ...
            {user.role === 'Super Admin' && (
              <div>
                ...
                {errors.club && <div style={err}>{errors.club}</div>}
              </div>
            )}
            ...
            <div>
              ...
              {errors.time && <div style={err}>{errors.time}</div>}
            </div>
            ...
            <div>
              ...
              {errors.capacity && <div style={err}>{errors.capacity}</div>}
            </div>
          ...
          <div style={{ display:'flex', gap:12, marginTop:28, paddingTop:22, borderTop:'1px solid rgba(245,197,24,0.08)' }}>
            <GoldButton size="lg" onClick={handleSave} style={{ opacity: saving ? 0.7 : 1 }}>
              {saving ? '···' : form.status === 'Published' ? '⚡ Publish Event' : '💾 Save as Draft'}
            </GoldButton>
